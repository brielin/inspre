...
ENSG00000242485 <- c("ENSG00000188290", "ENSG00000078808", "ENSG00000176022")
keep_cells <- (obs$gene_transcript == target)|(obs$gene == ntc)
keep_gene <- (var$gene_id %in% genes)
X <- obs[keep_cells,]$gene_id != ntc
Y <- hfile_sc[['X']][keep_gene, keep_cells]

tsls_data <- as_tibble(t(Y))
colnames(tsls_data) <- var$gene_id[keep_gene]
tsls_data$X <- X
