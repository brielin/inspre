...
inspre_cold_function <- function(X){
  S_hat <- cor_w_se(X)$S_hat
  res <- inspre(
    S_hat, verbose = 1, nlambda = nlambda, lambda_min_ratio = lambda_min_ratio,
    alpha = 0, symmetrize = TRUE, warm_start = FALSE)
  return(list("theta" = res$V, "lambda" = res$lambda))
}

inspre_cold_mask_function <- function(X){
  S_hat <- cor_w_se(X)$S_hat
  res <- inspre(
    S_hat, verbose = 1, nlambda = nlambda, lambda_min_ratio = lambda_min_ratio,
...
methods = list("glasso_best" = glasso_function,
               ...
               # "inspre_cold" = inspre_cold_function,
               # "inspre_cold_mask" = inspre_cold_mask_function)
