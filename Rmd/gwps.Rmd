...
```{r}
load('/gpfs/commons/groups/knowles_lab/gwps/saved_rdata/response_vars_essential.Rdata')
```
...
fc_data <- left_join(normalized_group_counts, ntc_count, by = "gene_id") %>% filter(!is.na(ntc_count)) %>% mutate(fc = ntc_count/group_count, log_fc = log2(fc))
```
