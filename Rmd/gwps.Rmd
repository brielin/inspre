...
```{r}
k562_norm_pb_fn = '/gpfs/commons/home/bbrown/perturb_inspre/gwps_data/K562_essential_normalized_bulk_01.h5ad'
k562_norm_sc_fn = '/gpfs/commons/home/bbrown/perturb_inspre/gwps_data/K562_essential_normalized_singlecell_01.h5ad'
# k562_norm_pb_fn = '/gpfs/commons/home/bbrown/perturb_inspre/gwps_data/K562_gwps_normalized_bulk_01.h5ad'
# k562_norm_sc_fn = '/gpfs/commons/home/bbrown/perturb_inspre/gwps_data/K562_gwps_normalized_singlecell_01.h5ad'
...
as_tibble(beta) %>% ggplot(aes(x=ENSG00000260289)) + stat_density()
```
...
inst_data %>% ggplot(aes(x=control_exp, y=inst_effects)) + geom_point() + xlim(0,2)
inst_data %>% ggplot(aes(x=log2(cv), y=inst_Z)) + geom_point()
...
# save(normalized_group_counts, file = '/gpfs/commons/home/bbrown/ukbb_network/saved_rdata/normalized_counts_essential.Rdata')
...
lambda <- exp(seq(log(1), log(0.1), length.out = 10))
cde_res_wide <- fit_inspre(R_top, W = weights, verbose = 2, cv_folds = 10, ncores = 8, warm_start = FALSE, delta_target = 5e-4, lambda = lambda, its=50, min_nz = 1e-3)
save(cde_res_wide, file = "/gpfs/commons/home/bbrown/perturb_inspre/saved_rdata/cde_res_wide.Rdata")
```


```{r}
# Lambda chosen from above
cde_res <- fit_inspre(R_top, W = weights, verbose = 2, cv_folds = 0, ncores = 1, warm_start = FALSE, delta_target = 5e-4, lambda = 0.6, its=500)
save(cde_res, file = "/gpfs/commons/home/bbrown/perturb_inspre/saved_rdata/cde_res.Rdata")
```


```{r}
# THIS IS WRONG IT DIDNT GET SAVED
load("/gpfs/commons/home/bbrown/perturb_inspre/saved_rdata/cde_res.Rdata")
col_phenos <- colnames(cde_res$R_hat)
row_phenos <- rownames(cde_res$R_hat)
data <- data_top %>% dplyr::filter(Exposure %in% phenos, Outcome %in% phenos)
...
```{r}
load("/gpfs/commons/home/bbrown/perturb_inspre/saved_rdata/top_genes_data.Rdata")
load("/gpfs/commons/home/bbrown/perturb_inspre/saved_rdata/top_genes_graph.Rdata")
```


```{r}
ggplot(data, aes(y=Exposure, x=Outcome)) + geom_tile(aes(fill=U_hat)) +
   scale_x_discrete(limits = col_phenos[plot_order], labels = NULL) +
   scale_y_discrete(limits = row_phenos[rev(plot_order)], labels = NULL) +
